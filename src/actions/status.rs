...
use crate::level::{decode_string, get_marker};
...
pub fn run() -> Result<(), String> {
    ...
    let old_objects = decoded_string[old_sep..].to_string();

    let new_sep = string.find(";").unwrap();
    let new_objects = string[new_sep..].to_string();

    let (added, removed) = compare_strings(&new_objects, &old_objects)?;
